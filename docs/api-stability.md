...
| `github.com/3leaps/gonimbus/pkg/provider`      | Stable       | Provider interface, metadata types, capability interfaces (`ConditionalWriteCapabilities` / `ConditionalCapabilityReporter`), optional revision-bound reads (`Revision` / `RevisionGetter`), and `ResolveConnectionPool` construction policy.     |
...
| `github.com/3leaps/gonimbus/pkg/reflow`        | Experimental | Reflow workflow engine: migrated stdin / positional-S3 execution, records, adaptive and dual-domain admission, collision/durability, provenance sidecars, and redaction helpers. Surface grew in v0.4.2; still Experimental, no Stable promotion. |
...
| `github.com/3leaps/gonimbus/pkg/runbudget`     | Experimental | Shared provider admission: quota-domain-keyed rate, per-class concurrency, per-domain copy permits, open bodies, memory, and throttle response.                                                                                                   |
